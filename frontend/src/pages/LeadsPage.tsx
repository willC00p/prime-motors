    ...
    <div className="space-y-6 p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Monitor application workflow pipeline</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={fetchLeads}
            className="px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-700 text-sm font-medium"
          >
            Refresh
          </button>
        </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {WORKFLOW_STAGES.slice(0, 5).map((stage) => (
            <div
              key={stage.id}
              className={`${stage.color} ${stage.textColor} p-4 rounded-lg shadow-sm`}
            >
              <div className="text-2xl font-bold">{summary[stage.id as keyof Summary]}</div>
              <div className="text-xs mt-1">{stage.label}</div>
            </div>
          ))}
          <div className="bg-red-100 text-red-800 p-4 rounded-lg shadow-sm">
            <div className="text-2xl font-bold">{summary.SLA_EXCEEDED}</div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm flex gap-4">
        ...
        <select
          ...
          className="px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm cursor-pointer"
        >
